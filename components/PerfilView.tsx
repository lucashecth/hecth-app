...
export function PerfilView({ onVoltar, alunoDb }: PerfilViewProps) {
  ...
  async function handleTrocarFoto(event: React.ChangeEvent<HTMLInputElement>) {
    try {
      ...
      const fileName = `perfil-${alunoDb.id}-${Date.now()}.${fileExt}`;
      ...
      // 3. Atualiza o cadastro do aluno
      await supabase.from('alunos').update({ foto_url: novaUrl }).eq('id', alunoDb.id);
      
      alert("✅ Foto atualizada! Recarregue o app para ver a mudança.");
      window.location.reload(); // Recarrega para puxar a foto nova em todo o app
  ...
  return (
    <div className="animacao-entrada w-full pb-20 pt-4">
      ...
      <div className="px-5">
        <div className="bg-[#121212] border border-white/5 rounded-3xl p-6 flex flex-col items-center text-center relative overflow-hidden">
          <div className="w-24 h-24 rounded-full overflow-hidden border border-white/10 mb-4 bg-white/5 shrink-0 relative">
            ...
            {uploading && (
              <div className="absolute inset-0 bg-black/50 flex items-center justify-center">
                <span className="text-[10px] text-white font-black animate-pulse">Enviando...</span>
              </div>
            )}
          </div>
          ...
          <input type="file" accept="image/*" className="hidden" ref={fileInputRef} onChange={handleTrocarFoto} />
          <button 
            onClick={() => fileInputRef.current?.click()}
            disabled={uploading}
            className="bg-white/5 border border-white/10 px-6 py-3 rounded-xl text-xs font-black uppercase tracking-widest text-white/70 active:bg-white/10 transition-all"
          >
            {uploading ? 'Aguarde...' : 'Alterar Foto'}
          </button>
        </div>
