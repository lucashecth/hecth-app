...
export function AdminAlunosView({ onVoltar }: AdminAlunosViewProps) {
  ...
  <div className="animacao-entrada min-h-screen pb-20 w-full">
    {/* Header - Mantendo um pequeno recuo apenas nos ícones para não colarem na quina da tela */}
    ...
    {/* Busca - Agora ocupando 100% da largura útil */}
    <div className="px-0 mb-6">
      <input 
        type="text" 
        placeholder="BUSCAR NOME DO ATLETA..." 
        className="w-full bg-[#121212] border-y border-white/10 px-6 py-5 text-sm font-black uppercase tracking-widest outline-none focus:border-[#ef3340]/50 text-white"
        value={busca} 
        onChange={(e) => setBusca(e.target.value)}
      />
    </div>

    {/* Filtros - Mantendo um respiro mínimo lateral para não grudar no vidro do celular */}
    ...
    <div className="flex flex-col gap-1"> {/* Gap menor para parecer uma lista contínua e profissional */}
      {loading ? (
