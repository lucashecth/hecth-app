...
export function AdminAlunosView({ onVoltar }: AdminAlunosViewProps) {
  ...
  function CardAluno({ aluno, mostrarDia }: { aluno: any, mostrarDia?: boolean }) {
    ...
    return (
      /* O card continua w-full para preencher todo o espaço que dermos a ele */
  ...
  return (
    /* A MÁGICA AQUI: -mx-5 "anula" a margem do arquivo pai e estica a div para as bordas reais do celular */
    <div className="animacao-entrada min-h-screen pb-20 w-auto -mx-5"> 
      
      {/* Header com px-5 para ficar alinhado certinho com o resto do seu app */}
      ...
      {/* Busca e Filtros também com px-5 para manter o alinhamento central */}
      <div className="px-5">
        <input 
      ...
      {/* LISTA: Aqui usamos px-2. Como "anulamos" o pai lá em cima, os botões agora vão esticar até quase tocar as bordas amarela da sua foto! */}
          ...
          [10, 15, 20].map(dia => {
            ...
            return (
              <div key={dia} className="mb-10">
                <div className="flex items-center gap-3 mb-6 px-4"> {/* Alinha a tag "Dia 10" */}
                ...
                {/* Aqui os cards de vencimento também esticam */}
                <div className="flex flex-col gap-4 px-0">{alunosDoDia.map(aluno => <CardAluno key={aluno.id} aluno={aluno} mostrarDia />)}</div>
              </div>
            );
          })
        }
