      ...
      <div className={`w-full bg-[#121212] border rounded-2xl p-4 flex items-center justify-between transition-all active:scale-[0.98] ${aluno.mensalidade_paga ? 'border-green-500/30' : 'border-white/5'}`}>
        ...
        <div className="flex items-center gap-2">
          <button 
            ...
            className={`px-3 py-2 rounded-xl border ${aluno.mensalidade_paga ? 'bg-green-500/10 border-green-400/50' : 'bg-white/5 border-white/10'}`}
          >
    ...
    <div className="animacao-entrada min-h-screen pb-20 pt-4 w-full">
      ...
      <div className="px-4 mb-4">
        <input 
          ...
          className="w-full bg-[#121212] border border-white/10 rounded-xl px-4 py-3 text-sm font-black uppercase outline-none"
          value={busca}
      ...
      <div className="flex gap-2 mb-6 px-4">
        <button 
          ...
          className={`flex-1 py-3 rounded-xl text-xs font-black uppercase ${filtro === 'todos' ? 'bg-white text-black' : 'bg-white/5 text-white/40'}`}
        >
          TODOS
        </button>
        <button 
