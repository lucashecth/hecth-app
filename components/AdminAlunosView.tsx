...
export function AdminAlunosView({ onVoltar }: AdminAlunosViewProps) {
  ...
  // Lógica de Níveis (mesma de antes)
  ...
  function CardAluno({ aluno, mostrarDia }: { aluno: any, mostrarDia?: boolean }) {
    ...
    return (
      /* w-full aqui garante que ele tente ocupar tudo */
  ...
  return (
    /* AQUI ESTÁ A CHAVE: px-0 para encostar nas bordas laterais */
    <div className="animacao-entrada min-h-screen pb-20 px-0 w-full"> 
      
      {/* O Header a gente mantém um px-4 para não sumir o botão de voltar no limite da tela */}
      ...
      {/* Busca e Filtros também em px-4 para manter o alinhamento visual do formulário */}
      <div className="px-4">
        <input 
      ...
      {/* LISTA: Aqui os botões vão encostar de um lado ao outro se usarmos px-2 ou px-1 */}
          ...
          [10, 15, 20].map(dia => {
            ...
            return (
              <div key={dia} className="mb-10">
                ...
                <div className="flex flex-col gap-4 px-2">{alunosDoDia.map(aluno => <CardAluno key={aluno.id} aluno={aluno} mostrarDia />)}</div>
              </div>
            );
          })
        }
