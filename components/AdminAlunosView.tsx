...
export function AdminAlunosView({ onVoltar }: AdminAlunosViewProps) {
  ...
  function CardAluno({ aluno, mostrarDia }: { aluno: any, mostrarDia?: boolean }) {
    ...
    return (
      <div className={`w-full bg-[#121212] border rounded-3xl p-4 flex items-center justify-between transition-all active:scale-[0.98] ${aluno.mensalidade_paga ? 'border-green-500/30 shadow-[0_0_15px_rgba(34,197,94,0.05)]' : 'border-white/5'}`}>
          ...
          <div className="text-left">
            ...
            <div className="flex flex-wrap gap-1 mt-1">
              {mostrarDia ? (
                ...
                tags.map(tag => (
                  <span key={tag} className={`text-[8px] font-black uppercase tracking-widest px-1.5 py-0.5 rounded border italic ${aluno.mensalidade_paga ? 'text-green-400 border-green-400/30' : 'text-[#ef3340] border-[#ef3340]/20'}`}>
                    {tag}
                  </span>
                ))
              )}
        ...
        <div className="flex items-center gap-3">
          <button 
            ...
            className={`px-3 py-2 rounded-xl border transition-all ${aluno.mensalidade_paga ? 'bg-green-500/10 border-green-400/50' : 'bg-white/5 border-white/10'}`}
          >
          ...
          <button 
            ...
            className={`w-10 h-10 rounded-2xl flex items-center justify-center transition-all ${aluno.mensalidade_paga ? 'bg-green-500 text-black shadow-lg shadow-green-500/20' : 'bg-[#1a1a1a] text-white/10 border border-white/5'}`}
          >
  ...
  return (
    <div className="animacao-entrada min-h-screen pb-20 px-2"> {/* Margem lateral mínima para os botões crescerem */}
      ...
      <div className="flex gap-2 mb-8">
        <button onClick={() => setFiltro('todos')} className={`flex-1 py-4 rounded-xl text-[10px] font-black uppercase tracking-widest ${filtro === 'todos' ? 'bg-white text-black' : 'bg-white/5 text-white/40 border border-white/5'}`}>Todos A-Z</button>
        <button onClick={() => setFiltro('vencimento')} className={`flex-1 py-4 rounded-xl text-[10px] font-black uppercase tracking-widest ${filtro === 'vencimento' ? 'bg-[#ef3340] text-white' : 'bg-white/5 text-white/40 border border-white/5'}`}>Por Vencimento</button>
      </div>

      <div className="flex flex-col gap-3"> {/* Volta o espaço entre os botões */}
        {loading ? <p className="text-center py-10 animate-pulse font-black uppercase text-[10px] text-white/20 italic tracking-widest">Sincronizando...</p> : 
